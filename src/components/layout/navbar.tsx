...
const navLinks = [
  { id: 'features', labelKey: 'navFeatures' as const, icon: Sparkles },
  { id: 'specs', labelKey: 'navSpecs' as const, icon: Gauge },
  { id: 'pre-order', labelKey: 'navPreOrder' as const, icon: PackageCheck },
];
...
export const Navbar: React.FC = () => {
  ...
  const totalCartPrice = cart.reduce((acc, item) => acc + (language === 'vi' ? 9990000 : 399) * item.quantity, 0);
        ...
        <div className="max-w-300 mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="w-9 h-9 ml-1 flex items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors border border-slate-200/40 dark:border-slate-800/40 overflow-hidden"
              title={language === 'vi' ? 'Switch to English' : 'Chuyển sang Tiếng Việt'}
            >
              {language === 'vi' ? (
                <VN className="w-5 h-3.5 object-cover rounded-xs shadow-xs" />
              ) : (
                <US className="w-5 h-3.5 object-cover rounded-xs shadow-xs" />
              )}
            </button>
            ...
            <div className="w-screen max-w-md bg-white dark:bg-slate-950 shadow-2xl flex flex-col">
              <div className="px-6 py-6 border-b border-slate-100 dark:border-slate-900 flex items-center justify-between">
                <h2 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                  <ShoppingBag size={20} /> {t.cart} ({totalCartCount})
                </h2>
                <button
                  onClick={() => setIsCartOpen(false)}
                    ...
                    >
                      <div className="text-left">
                        <p className="font-semibold text-sm text-slate-800 dark:text-white">
                          AuraRing X ({translations[language][item.color === 'Matte Black' ? 'matteBlack' : item.color === 'Chrome Silver' ? 'chromeSilver' : 'roseGold'] || item.color})
                        </p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                          Size: {item.size} • Qty: {item.quantity}
                        </p>
                        <p className="text-sm font-bold text-[#004ac6] dark:text-blue-400 mt-1">
                          {language === 'vi' ? `${(item.price * item.quantity * 25000).toLocaleString('vi-VN')} VNĐ` : `$${item.price * item.quantity} USD`}
                        </p>
                      </div>
                      <button
                ...
                <div className="px-6 py-6 border-t border-slate-100 dark:border-slate-900 bg-slate-50/50 dark:bg-slate-950/50 space-y-4">
                  <div className="flex items-center justify-between text-sm font-semibold text-slate-800 dark:text-white">
                    ...
                    <span className="text-lg font-bold text-[#004ac6] dark:text-blue-400">
                      {language === 'vi' ? `${(totalCartPrice * 25000).toLocaleString('vi-VN')} VNĐ` : `$${totalCartPrice} USD`}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button
                      onClick={() => {
                        addToast(t.checkoutSimulated, 'info');
                        clearCart();
                        setIsCartOpen(false);
                      }}
